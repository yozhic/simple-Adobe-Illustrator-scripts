...
/********************************************************
  Description .....: Find/Replace text in layer names
  Version .........: 1.2.0
  C.Date / M.Date .: 05.07.2020 / 07.07.2020
...
function main()
{
	var sTitle = "Change Layers Listing";
	...
	var btnReplace = btnGroup.add("button", undefined, "Replace", {name: "btnReplace"});
		...
		btnReplace.helpTip = 'Alt+Click: Toggle Layers panel visibility';
		...
		if (regexpCBox.value === true) {
			...
			if (findString.text === "") {
				...
				for (i = 0; i < doc.layers.length; i++) {
					doc.layers[i].name = doc.layers[i].name.replace (findStrRegExp, replaceString.text);
						if (wcardsCBox.value === true) {
							doc.layers[i].name = doc.layers[i].name.replace (/\\c1/g, counter(i + 1, 1));
							doc.layers[i].name = doc.layers[i].name.replace (/\\c2/g, counter(i + 1, 2));
							doc.layers[i].name = doc.layers[i].name.replace (/\\c3/g, counter(i + 1, 3));
						}
				}
			...
			if (findString.text === "") {
				...
				for (i = 0; i < doc.layers.length; i++) {
					doc.layers[i].name = doc.layers[i].name.replace (findString.text, replaceString.text);
						if (wcardsCBox.value === true) {
							doc.layers[i].name = doc.layers[i].name.replace (/\\c1/g, counter(i + 1, 1));
							doc.layers[i].name = doc.layers[i].name.replace (/\\c2/g, counter(i + 1, 2));
							doc.layers[i].name = doc.layers[i].name.replace (/\\c3/g, counter(i + 1, 3));
